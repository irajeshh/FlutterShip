  ...
  Widget page(IconLabel iconLabel){
    return Center(
      child: ListTile(
        ...
        subtitle: Txt(text: "${iconLabel.label} page is Selected",
...
switch (currentIndex) {
      case 0: selectedSubMenuWidget = subMenuWidget0();
      break;

      case 2: selectedSubMenuWidget = subMenuWidget2();
      break;

      case 3: selectedSubMenuWidget = subMenuWidget3();
      break;
    
      default: selectedSubMenuWidget =  Container(height:0);
    }
    return Expanded(
  ...
  Widget subMenuWidget0(){
    List<String> tags = [
      "All", "Cinema", "Cars", "Languages", "Flutter", "Android", "Love", "Inspiration",
    ];
  ...
  Widget topMenuButton(String topMenuTitle){
    ...
    if(showTopMenusSubMenus==false){
      if(currentIndex==1 || currentIndex==2) borderRadius = BorderRadius.only(topLeft: Radius.circular(8),topRight: Radius.circular(8));
 ...
 Widget subMenuWidget(List<String> submenus){
  ...
  static List<IconLabel> iconLables = [
    ...
    CDM(Icons.subscriptions, "Subscriptions Menu", ["Old","A-z","Z-A",]),
    CDM(Icons.video_library, "Library Menu", ["History","Liked","Watchlater",]),
  ];
  ...
  void expandHeight(int index){
    currentIndex = index;
      switch (index) {
       case 0: totalheight = 70 + 24.0;
       break;
       case 2: totalheight = 70 + 120.0;
       break;
       case 3: totalheight = 70 + 53.0;//45 for list view and 8 for vertical padding,
       break; 
       default: totalheight = 70;
     }
    setState((){});
  }
