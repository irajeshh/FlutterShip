...
class _ImgState extends State<Img> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
   super.build(context);
  BorderRadius borderRadius = BorderRadius.circular(widget.radius ?? 8);
   return Inkk(
  ...
  Widget imgWidget() {
    Widget _imgWidget = CachedNetworkImage(
     ...
     return Stack(
          ...
          children: <Widget>[
            widget.enableRippleEffect ? Inkk(
                    onTap: widget.onTap ?? () {},
  ...
  void viewImageFn(){
      Widgets.instance.push(Scaffold(
        backgroundColor: Colors.black,
        body: Center(
          child: CachedNetworkImage(
              imageUrl: widget.imgUrl,
              width: double.maxFinite,
              placeholder: (c,s,)=>placeholder(c,s,s),
              errorWidget: errorWidget,
          ),
        ),
      ), context);
